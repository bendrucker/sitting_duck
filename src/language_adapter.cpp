...
string LanguageAdapter::ExtractQualifiedIdentifierName(TSNode node, const string &content) const {
	...
	vector<string> patterns = {
	    "qualified_identifier", "scoped_identifier", "nested_identifier",
	...
	vector<string> container_patterns = {"function_declarator", "method_declarator", "declarator", "class_body",
	                                     "interface_body"};
...
string LanguageAdapter::ExtractFunctionNameFromText(TSNode node, const string &content) const {
	...
	if (function_name.empty() || (!isalpha(function_name[0]) && function_name[0] != '_' && function_name[0] != '~')) {
		return "";
	}
...
void LanguageAdapterRegistry::ValidateLanguageABI(const LanguageAdapter *adapter) const {
	...
	try {
		...
		if (!parser) {
			throw InvalidInputException("Language adapter for '" + language + "' failed to create parser");
		}
